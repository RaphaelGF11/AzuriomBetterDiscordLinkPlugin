...
class UserController extends Controller
{
     ...
     * to log in - see DiscordLoginServiceProvider::registerUnlinkGuard()).
    ...
    public function forceUnlinkDiscord(User $user): Response
    {
        ...
        if (! $user->discord_login_passwordless) {
            $user->forceFill(['discord_login_passwordless' => true])->saveQuietly();
        }
    ...
    public function sendDirectMessage(Request $request, User $user): Response
    {
        ...
        if (! DiscordBotClient::sendDirectMessage($account->discord_user_id, $data['content'])) {
            return to_route('admin.users.edit', $user)
                ->with('error', trans('discord-login::admin.tools.dm.failed'));
        }

        return to_route('admin.users.edit', $user)
            ->with('success', trans('discord-login::admin.tools.dm.sent'));
    }
    ...
    public function sendRecoveryPassword(Request $request, User $user): Response
    {
        ...
        if (! DiscordBotClient::sendDirectMessage($account->discord_user_id, trans('discord-login::messages.tools.recovery_dm', [
            'site' => site_name(),
            'password' => $password,
        ]))) {
            return to_route('admin.users.edit', $user)
        ...
        return to_route('admin.users.edit', $user)
            ->with('success', trans('discord-login::admin.tools.recovery_password.sent'));
    }
    ...
    public function refreshDiscordInfo(User $user): Response
    {
        ...
        if ($response->failed()) {
            return to_route('admin.users.edit', $user)
                ->with('error', trans('discord-login::admin.tools.refresh.failed'));
        }
        ...
        if (setting('discord-login.sync_avatar', false)) {
        ...
        return to_route('admin.users.edit', $user)
            ->with('success', trans('discord-login::admin.tools.refresh.done'));
    }
    ...
    public function send2faRecoveryCodes(User $user): Response
    {
        ...
        if (! DiscordBotClient::sendDirectMessage($account->discord_user_id, trans('discord-login::messages.tools.recovery_codes_dm', [
            'site' => site_name(),
            'codes' => implode("\n", $codes),
        ]))) {
            return to_route('admin.users.edit', $user)
        ...
        return to_route('admin.users.edit', $user)
            ->with('success', trans('discord-login::admin.tools.recovery_codes.sent'));
    }
